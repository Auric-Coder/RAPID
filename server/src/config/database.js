...
/**
 * Repairs user scope_id values against the live Supabase rows.
 *
 * seedUsers carry scope_id values generated by THIS process's in-memory seed
 * (a fresh crypto.randomUUID() every boot), while states/bases persist in
 * Supabase from an earlier boot with different ids. The user upsert above
 * re-resolves organisation_id by code, but scope_id passed through unchanged
 * - so a STATE_COMMANDER's scope_id matched no live state, resolveAllowedBaseIds
 * returned nothing, and those users saw an empty fleet, no bases and no
 * incidents: the entire application was blank for them.
 *
 * Runs after the geography seed so live states are guaranteed to exist, and is
 * idempotent - it only writes rows whose scope_id is actually wrong, so a
 * healthy database performs zero writes.
 */
