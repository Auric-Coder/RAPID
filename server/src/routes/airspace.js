...
router.post('/zones', requireRole(...ZONE_WRITE_ROLES), async (req, res) => {
  try {
    ...
    // Step 5: keep GET /api/geo/map-config correct immediately rather than
    // waiting out its 60s TTL — this is safety-relevant no-fly-zone data.
    // Clearing every state's entry (not just this zone's) is simplest and
    // still cheap: there are only 2 states, and it avoids a state_id->code
    // lookup here.
...
router.patch('/zones/:id', requireRole(...ZONE_WRITE_ROLES), async (req, res) => {
  try {
    ...
    invalidate('geo:map-config:'); // Step 5 — see note in the POST handler above
    res.json(zone);
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
